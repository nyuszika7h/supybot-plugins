...
conf.registerChannelValue(MCStatus, 'separator',
        registry.String(' | ', _("""This text is inserted between service-status
        pairs.""")))
...
conf.registerChannelValue(MCStatus.service, 'suffix',
        registry.String(': ', _("""This text is appended to each service's
        name.""")))
conf.registerGroup(MCStatus, 'status')
conf.registerChannelValue(MCStatus.status, 'online',
        registry.String('\x0309ONLINE\x03', _("""This text is displayed when a
        service is online (green).""")))
conf.registerChannelValue(MCStatus.status, 'offline',
        registry.String('\x0304OFFLINE\x03', _("""This text is displayed when a
