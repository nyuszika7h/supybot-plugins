...
conf.registerGlobalValue(MCStatus, 'statusURL',
        registry.StringSurroundedBySpaces('http://status.mojang.com/check', _("""This is the URL
        that is queried for JSON data. You shouldn't change this unless you
        know what you are doing.""")))
conf.registerChannelValue(MCStatus, 'prefix',
        registry.StringSurroundedBySpaces('', _("""This text is prepended to the mcstatus
        command's output.""")))
conf.registerChannelValue(MCStatus, 'suffix',
        registry.StringSurroundedBySpaces('', _("""This text is appended to the mcstatus
        command's output.""")))
...
conf.registerChannelValue(MCStatus.service, 'prefix',
        registry.StringSurroundedBySpaces('', _("""This text is prepended to each service's
        name.""")))
conf.registerChannelValue(MCStatus.service, 'suffix',
        registry.StringSurroundedBySpaces(': ', _("""This text is appended to each service's
        name.""")))
conf.registerGroup(MCStatus, 'status')
conf.registerChannelValue(MCStatus.status, 'online',
        registry.StringSurroundedBySpaces('\x0309ONLINE\x03', _("""This text is displayed when a
        service is online (green).""")))
conf.registerChannelValue(MCStatus.status, 'offline',
        registry.StringSurroundedBySpaces('\x0304OFFLINE\x03', _("""This text is displayed when a
